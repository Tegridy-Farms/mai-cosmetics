...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const parsed = LeadQuerySchema.safeParse(Object.fromEntries(url.searchParams.entries()));
    if (!parsed.success) {
    ...
    if (search && search.trim().length > 0) {
      ...
      whereParts.push(`(l.full_name ILIKE $${params.length} OR l.phone ILIKE $${params.length} OR l.email ILIKE $${params.length})`);
    }
    if (date_from) {
    ...
    return jsonResponse({ data: dataRows.rows, total, page, pageSize });
  } catch {
